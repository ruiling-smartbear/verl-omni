...
class DiffusionAgentLoopWorker:
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        # this rollout, so the step-level "Training Progress" bar alone leaves a
        # healthy run looking stalled between two of its updates.  Count the
        # samples as they finish so a slow rollout reads as slow rather than
        # invisible.  The label says which phase, because the same bar covers
        # the validation pass that runs on the last step.
        phase = "Validation" if is_validate else "Rollout"
        progress = tqdm(total=len(tasks), desc=phase, unit="sample", dynamic_ncols=True, leave=True)
