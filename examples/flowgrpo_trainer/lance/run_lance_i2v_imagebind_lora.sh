# Lance LoRA RL (image-to-video), vllm_omni rollout (FlowGRPO) with an ImageBind reward
#
# Mirrors run_lance_t2v_imagebind_lora.sh, which is the text-to-video recipe.
# What differs:
#
#   * the dataset carries a reference frame per sample, bound to the sample by
#     the '<image>' marker the dataset layer substitutes (see
#     data_process/lance_i2v.py).  The rollout transport knows only media
#     modalities, so the frame arrives as multi_modal_data['image'] and the Lance
#     adapter renames it to 'first_frame', which is the key the i2v node reads:
#     under the plain key LancePipeline.forward would fall through to the
#     text-to-video node and drop the reference without an error.
#   * the prompt stays the bare caption.  The i2v node rebuilds its chat
#     template segments itself and takes only the instruction from the prompt,
#     so no vision marker belongs in the text.
#   * imagebind stays the reward, scoring the generated video against the
#     caption as in the text-to-video recipe.
#
# Prerequisite: the Lance bundle and an image-to-video dataset, for example::
#
#     python examples/flowgrpo_trainer/data_process/lance_i2v.py \
#         --model_path ~/models/bytedance-research/Lance \
#         --input_dir ~/data/lance_i2v --output_dir ~/data/lance_i2v/lance
...
pickscore_train_path=$WORKSPACE/data/lance_i2v/lance/train.parquet
pickscore_test_path=$WORKSPACE/data/lance_i2v/lance/test.parquet
...
# Video shape and schedule.  Lance caps H and W at max_latent_size * 16 = 1024.
...
python3 -m verl_omni.trainer.main_diffusion \
    ...
    trainer.experiment_name=lance_i2v_imagebind_lora \
