# Lance LoRA RL (image edit), vllm_omni rollout (FlowGRPO) with PickScore reward
#
# Mirrors run_lance_pickscore_lora.sh, which is the text-to-image recipe.  What
# differs:
#
#   * the dataset carries a reference image per sample, bound to the sample by
#     the '<image>' marker the dataset layer substitutes (see
#     data_process/lance_image_edit.py).  The pipeline dispatches on the request's
#     modality, so the adapter announces 'image' even though that is the default
#     for a single-stage engine; without it an image-edit request looks like
#     text-to-image and the reference is dropped without an error.
#   * the reward stays PickScore, as in the text-to-image recipe: the edit
#     produces one image, scored against the sample's target description.
#
# Prerequisite: the Lance bundle and an image-edit dataset, for example::
#
#     python examples/flowgrpo_trainer/data_process/lance_image_edit.py \
#         --model_path ~/models/bytedance-research/Lance \
#         --input_dir ~/data/lance_image_edit --output_dir ~/data/lance_image_edit/lance
...
pickscore_train_path=$WORKSPACE/data/lance_image_edit/lance/train.parquet
pickscore_test_path=$WORKSPACE/data/lance_image_edit/lance/test.parquet
...
python3 -m verl_omni.trainer.main_diffusion \
    ...
    trainer.experiment_name=lance_image_edit_pickscore_lora \
